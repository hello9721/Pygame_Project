...
class Game:
    ...
    def update(self):
        ...
        while len(self.platforms) < 6:
            ...
            p = Platform(self, random.randrange(0, SCRWIDTH - width),
    ...
    def events(self):
        ...
        for event in pg.event.get():
            ...
            self.player.keyboard(event)

            if self.player.rect.x < -5: self.player.rect.x = -5
            elif self.player.rect.x > SCRWIDTH - WIDTH + 5: self.player.rect.x = SCRWIDTH - WIDTH + 5
    
        self.player.jump()
        
        self.scr.fill((0,0,0))
        self.scr.blit(self.player.ball_img, self.player.rect)

    def draw(self):
        # Game Loop - Draw
        self.scr.fill(BGCOLOR)
        self.all_sprites.draw(self.scr)
        self.scr.blit(self.player.image, self.player.rect)
        self.draw_text(str(self.score), 22, WHITE, SCRWIDTH/2, 15)

        # *after* drawing everything, flip the display
        pg.display.flip()

    def show_start_screen(self):
        # game splash/start scr
        self.scr.fill(BGCOLOR)
        self.draw_text(TITLE, 48, WHITE, SCRWIDTH/2, SCRHEIGHT/4)
        self.draw_text("Arrows to move, Space to jump",
                       22, WHITE, SCRWIDTH/2, SCRHEIGHT/2)
        self.draw_text("Press a key to play",
                       22, WHITE, SCRWIDTH/2, SCRHEIGHT*3/4)
        self.draw_text("High Score : " + str(self.highscore),
                       22, WHITE, SCRWIDTH/2, 15)
        pg.display.flip()
        self.wait_for_key()
    ...
    def show_go_screen(self):
        ...
        if not self.running:
            return
        self.scr.fill(BGCOLOR)
        self.draw_text("GAME OVER", 48, WHITE, SCRWIDTH/2, SCRHEIGHT/4)
        ...
        if self.score > self.highscore:
            ...
            self.draw_text("NEW HIGH SCORE!",
                           22, WHITE, SCRWIDTH/2, SCRHEIGHT/2 + 40)
